                ...
                for line_num, line in enumerate(f, start=start_line):
                    ...
                    try:
                        ...
                        if error_count <= 10:  # Limit error messages
                            error_queue.put(f"Worker {worker_id}: Line {line_num}: {str(e)}")
    ...
    try:
        if input_format == 'csv':
            ...
            with open(file_path, 'r') as f:
                edge_iterator = (json.loads(line.strip()) for line in f)
...
def stream_edges_parallel(file_path, topic, bootstrap_server, config, num_workers, input_format='json'):
    ...
    if input_format == 'csv':
        ...
        csv_chunks = build_csv_byte_chunks(file_path, num_workers)
        if not csv_chunks:
            print("  No CSV data rows found.")
            return
        num_workers = len(csv_chunks)
        for i, (s, e) in enumerate(csv_chunks):
    ...
    for i in range(num_workers):
        if input_format == 'csv':
            start_offset, end_offset = csv_chunks[i]
            start_line = 0
            end_line = 0
        else:
            start_line = i * lines_per_worker
            end_line = start_line + lines_per_worker if i < num_workers - 1 else total_lines
            start_offset = 0
            end_offset = 0
        ...
        workers.append(p)
        if input_format != 'csv':
            print(f"    Worker {i}: lines {start_line:,} to {end_line:,}")
