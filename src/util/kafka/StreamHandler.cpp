...
static std::string getStreamHandlerTempDir() {
    std::string tempDir = Utils::getHomeDir() + "/.jasminegraph/tmp/streamhandler";
...
static uint32_t discoverLatestSnapshotIdOnWorker(const JasmineGraphServer::worker& worker, int graphId,
    ...
    while (std::getline(filesStream, remoteFile)) {
        ...
        std::string tempFile = getStreamHandlerTempDir() + "/snapshot_meta_" +
...
void StreamHandler::flushWorkerBatch(int workerId, bool force) {
    std::vector<std::string> edges;
    {
        ...
        edges = std::move(workerBatches[workerId]);
