...
static void history_bfs_command(int connFd, bool *loop_exit_p);
...
void* instanceservicesession(void* dummyPt) {
    ...
    while (!loop_exit) {
        ...
        } else if (cmd.compare(JasmineGraphInstanceProtocol::HISTORY_TRIANGLES) == 0) {
            history_triangles_command(connFd, &loop_exit);
        } else if (cmd.compare(JasmineGraphInstanceProtocol::HISTORY_PAGERANK) == 0) {
            history_pagerank_command(connFd, &loop_exit);
        } else if (cmd.compare(JasmineGraphInstanceProtocol::HISTORY_BFS) == 0) {
            history_bfs_command(connFd, &loop_exit);
        ...
        } else if (cmd.compare(JasmineGraphInstanceProtocol::GRAPH_STREAM_BATCH_START) == 0) {
            graph_stream_batch_start_command(connFd, streamHandler, &loop_exit);
        } else if (cmd.compare(JasmineGraphInstanceProtocol::WORKER_DIRECT_KAFKA_STREAM) == 0) {
...
void JasmineGraphInstanceService::trainPartition(string trainData) {
    ...
    std::string command = path +
                          "python3 -m unsupervised_train >  "
                          "/home/ubuntu/software/jasminegraph/logs/unsupervised_train" +
...
static void history_triangles_command(int connFd, bool *loop_exit_p) {
    ...
    int partitionIdInt_signed = std::stoi(partitionId);

    if (partitionIdInt_signed == -1) {
        std::string stagedSnapshotDir = Utils::read_str_trim_wrapper(connFd, data, INSTANCE_DATA_LENGTH);
        stagedSnapshotDir = Utils::expand_path(stagedSnapshotDir);
        if (!Utils::send_str_wrapper(connFd, JasmineGraphInstanceProtocol::OK)) {
            *loop_exit_p = true;
            return;
        }
        ...
        TemporalTriangleResult result = HistoryTriangles::countTrianglesAtSnapshot(
            graphIdInt, snapshotId, stagedSnapshotDir);

        if (!sendUint64(connFd, result.triangleCount) ||
            !sendUint64(connFd, result.rawEdges) ||
            !sendUint64(connFd, result.uniqueEdges) ||
            !sendUint32(connFd, result.partitionsProcessed) ||
            !sendUint64(connFd, static_cast<uint64_t>(result.durationMs))) {
            *loop_exit_p = true;
        }
        return;
    }

    uint32_t partitionIdInt = static_cast<uint32_t>(partitionIdInt_signed);
...
static void history_pagerank_command(int connFd, bool *loop_exit_p) {
    ...
    if (partitionIdInt_signed == -1) {
        ...
        HistoryPageRankResult result = HistoryPageRank::computePageRankAtSnapshot(
            graphIdInt, snapshotId, stagedSnapshotDir, topK, maxIterations, dampingFactor);

        uint32_t topKToSend = result.rankedNodes.size();
        if (!sendUint32(connFd, topKToSend)) {
            *loop_exit_p = true;
            return;
        }

        for (const auto& [nodeId, score] : result.rankedNodes) {
        ...
        if (!sendUint64(connFd, result.rawEdges) ||
            !sendUint64(connFd, static_cast<uint64_t>(result.durationMs))) {
            *loop_exit_p = true;
        }
        return;
...
static void history_bfs_command(int connFd, bool *loop_exit_p) {
    ...
    string sourceNode = Utils::read_str_trim_wrapper(connFd, data, INSTANCE_DATA_LENGTH);
    if (!Utils::send_str_wrapper(connFd, JasmineGraphInstanceProtocol::OK)) {
        *loop_exit_p = true;
        return;
    }

    string maxDepthText = Utils::read_str_trim_wrapper(connFd, data, INSTANCE_DATA_LENGTH);
    if (!Utils::send_str_wrapper(connFd, JasmineGraphInstanceProtocol::OK)) {
        *loop_exit_p = true;
        return;
    }
    ...
    string traceContext = Utils::read_str_trim_wrapper(connFd, data, INSTANCE_DATA_LENGTH);
    OpenTelemetryUtil::receiveAndSetTraceContext(traceContext, "history bfs traversal");
    OTEL_TRACE_FUNCTION();
    ...
    int graphIdInt = std::stoi(graphID);
    int partitionIdInt_signed = std::stoi(partitionId);
    int maxDepth = std::stoi(maxDepthText);
    ...
    if (partitionIdInt_signed == -1) {
        ...
        HistoryBFSResult result = HistoryBFS::runBFSAtSnapshot(
            graphIdInt, snapshotId, sourceNode, maxDepth, stagedSnapshotDir);

        if (!sendUint64(connFd, static_cast<uint64_t>(result.visitedNodes)) ||
            !sendUint64(connFd, static_cast<uint64_t>(result.totalEdges)) ||
            !sendUint32(connFd, result.totalNodes) ||
            !sendUint32(connFd, static_cast<uint32_t>(result.partitionsProcessed)) ||
            !sendUint64(connFd, static_cast<uint64_t>(result.durationMs)) ||
            !sendUint32(connFd, static_cast<uint32_t>(result.outputPath.size())) ||
            !sendAll(connFd, result.outputPath.data(), result.outputPath.size())) {
            *loop_exit_p = true;
        }
        return;
    }
...
static void processFile(string fileName, bool isLocal, InstanceStreamHandler& handler, bool isEmbedGraph) {
    ...
    std::unique_lock<std::mutex> lock(handler.mapLock_);
