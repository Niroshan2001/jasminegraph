...
void PageRankExecutor::doPageRank(std::string graphID, double alpha, int iterations, string partition, string host,
    ...
    int sockfd;
    std::string dataBuffer(DATA_BUFFER_SIZE, '\0');
    ...
    string response = Utils::read_str_trim_wrapper(sockfd, dataBuffer.data(), FRONTEND_DATA_LENGTH);
    ...
    response = Utils::read_str_trim_wrapper(sockfd, dataBuffer.data(), FRONTEND_DATA_LENGTH);
    if (response.compare(JasmineGraphInstanceProtocol::OK) == 0) {
        pageRank_logger.info("Received : " + JasmineGraphInstanceProtocol::OK);
    } else {
        pageRank_logger.error("Error reading from socket");
